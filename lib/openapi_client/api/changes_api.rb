...
module OpenapiClient
  class ChangesApi
    ...
    # @param change_type [ChangeType] Type of changes to query.
    # @param item_type [ItemType] Type of items to search in. If item_type is show, you can use show_type parameter to only search for movies or series. 
    ...
    def get_changes(country, change_type, item_type, opts = {})
      data, _status_code, _headers = get_changes_with_http_info(country, change_type, item_type, opts)
      data
    end
    ...
    # @option opts [OrderDirection] :order_direction Determines whether to order the results in ascending or descending order. 
    # @option opts [String] :output_language [ISO 639-1 code](https://en.wikipedia.org/wiki/ISO_639-1) of the output language. Determines in which language the output  will be in.  (default to 'en')
    # @return [Array<(ChangesResult, Integer, Hash)>] ChangesResult data, response status code and response headers
    def get_changes_with_http_info(country, change_type, item_type, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ChangesApi.get_changes ...'
      end
      # verify the required parameter 'country' is set
      if @api_client.config.client_side_validation && country.nil?
        fail ArgumentError, "Missing the required parameter 'country' when calling ChangesApi.get_changes"
      end
      # verify the required parameter 'change_type' is set
      if @api_client.config.client_side_validation && change_type.nil?
        fail ArgumentError, "Missing the required parameter 'change_type' when calling ChangesApi.get_changes"
      end
      # verify the required parameter 'item_type' is set
      if @api_client.config.client_side_validation && item_type.nil?
        fail ArgumentError, "Missing the required parameter 'item_type' when calling ChangesApi.get_changes"
      end
      ...
      query_params[:'change_type'] = change_type
      ...
      query_params[:'include_unknown_dates'] = opts[:'include_unknown_dates'] if !opts[:'include_unknown_dates'].nil?
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'ChangesResult'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['X-Rapid-API-Key']

      new_options = opts.merge(
        :operation => :"ChangesApi.get_changes",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ChangesApi#get_changes\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
